...
// Mapping sources (merged in order; later files override earlier ones)
const MAP_FILES = [
  path.resolve(ROOT, 'data', 'stadiums.json'),            // manual overrides (optional)
  path.resolve(ROOT, 'data', 'stadiums-extra.json'),      // optional
  path.resolve(ROOT, 'data', 'stadiums.generated.json')   // auto-fetched (this run)
];
...
function main() {
  ...
  for (const t of teams) {
    ...
    const exact = key(t.league_code, t.team_name);
    const fb = `team:${norm(t.team_name)}`;

    const val = map[exact] || fallback[fb];
  ...
  if (check && failOnMissing && missing > 0) {
    console.error(`ERROR: ${missing} teams still missing stadiums. Add mappings (manual or generated) and re-run.`);
    process.exit(1);
  }
}
