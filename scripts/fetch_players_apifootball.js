...
// - better logging of paging
// - fallbacks: /players (season) -> /players (no season) -> /players/squads or /teams squad
// - leaves image_url from API (p.photo) when present
// usage: node scripts/fetch_players_apifootball.js
...
async function getPlayersFromPlayersEndpoint(teamId, season, withSeason = true) {
  ...
  const all = [];
  while (true) {
    const params = { team: String(teamId), page: String(page) };
    ...
    console.log(`    /players ${withSeason ? `season=${season} ` : ''}team=${teamId} page=${cur}/${total} items=${items.length}`);
    items.forEach(r => {
      const p = r.player || {};
...
async function getPlayersFromSquadsFallback(teamId) {
  ...
  try {
    ...
    // json.response might be [{ team: {...}, players: [...] }]
    const resp = json.response || [];
    if (Array.isArray(resp) && resp.length) {
      // find players array (support both shapes)
...
async function getPlayersForTeam(teamId, season) {
  // 1) try /players with season (this is what you already used)
  try {
