 ...
 * - Do NOT move or replace the existing TEAM wheel canvas. Keep it in its left slot.
 * - Create a separate player wheel canvas dynamically (wheelPlayer + fxPlayer) inside the right-hand playerView
 *   when PLAYER mode is first used. This keeps TEAM wheel intact at all times.
 * - drawWheel() renders to the appropriate canvas depending on MODE.
 * - loadPlayers() normalizes players so they can be filtered by league (left chips). When PLAYER mode is active,
 *   the left filters determine which players appear (players whose league_code matches selected league chips).
 * - Player images use image_url (fallbacks) and are shown on player wheel slices when optLogo (Show logo) is enabled.
 * - All other UI (chips, history, modal, reveal) preserved.
 *
 * How to use:
 * - Overwrite your deployed app.js with this file and hard-refresh (Ctrl/Cmd+Shift+R).
 * - Click PLAYER; the player wheel will be created in the right box and populated with players filtered by
 *   the left-hand league chips (select "Premier League" to show Premier League players).
 *
 * NOTE: This file is defensive: if your HTML already contains a #wheelPlayer or #fxPlayer, it will reuse them.
 * If not, it creates them dynamically so you don't need to edit HTML.
 */
...
/* Wheel canvas(es):
   - team wheel is expected to exist as #wheel in the left column already (do not remove)
   - player wheel (#wheelPlayer/#fxPlayer) will be created dynamically inside playerView if missing */
...
function getLogo(url, onLoad) {
  ...
  img.onload = () => {
    ...
    // redraw both wheels defensively
...
function getFilteredPlayers() {
  ...
  // Filter players by active league chips. This matches the user's "filter by Premier League" requirement:
  const active = Array.from(chipsWrap.querySelectorAll('input:checked')).map(i => i.value);
  if (active.length === 0) return PLAYERS.slice();
  return PLAYERS.filter(p => {
    // If player's league_code matches selected league codes -> include
    if (p.league_code && active.includes(p.league_code)) return true;
    // Fallback: if meta.club matches a team whose league_code is in active
...
function ensurePlayerCanvas() {
  ...
  // If HTML already contains #wheelPlayer, reuse; otherwise create
  ...
  if (!wheelPlayer) {
    ...
    // insert before the top of playerView content
...
function drawOnCanvas(canvas, fxCanvas, data, currentAngle, selectedIdx, isPlayerMode) {
  ...
  for (let i = 0; i < N; i++) {
    ...
    if (canShowLogo) {
      ...
      const img = getLogo(imgUrl, () => requestAnimationFrame(() => { drawOnCanvas(canvas, fxCanvas, data, currentAngle, selectedIdx, isPlayerMode); }));
      if (img && img.complete) {
...
function spinPlayer() {
  if (spinning) return;
  if (!PLAYERS) { // try load if not present
    loadPlayers().then(() => {
      // fall through to spin once loaded
      spinPlayer();
    }).catch(() => {
      if (currentText) currentText.textContent = 'No players available.';
    });
    return;
  }
  const data = getFilteredPlayers();
...
function applyRevealByKey(key, el, enabled, btnId, labelText) {
  ...
  btn.addEventListener('click', (e) => {
    ...
    modalRevealState[key] = true; unblurElement(el); btn.remove();
  }, { passive: false });
  ...
  for (const url of urls) {
    try {
      ...
      PLAYERS = (Array.isArray(data) ? data : []).map(p => ({
        ...
        league_code: p.league_code || p.league || (p.club ? (TEAMS.find(t => (t.team_name || '').toLowerCase().includes((p.club||'').toLowerCase()))?.league_code || 'PLAYER') : 'PLAYER'),
        stadium: p.stadium || '',
        meta: p
      }));
      return PLAYERS;
