...
function fitSingleLine(ctx, text, {maxWidth, targetPx, minPx=9, maxPx=24, weight=800}){
  ...
  while (px > minPx){
    px--; ctx.font = `${weight} ${px}px Inter, system-ui, sans-serif`;
    if (ctx.measureText(text).width <= maxWidth) return {text, fontPx: px};
  }
...
/* ---------- Labels / presets ---------- */
const LEAGUE_LABELS={AUT:"Austrian Bundesliga",BEL:"Jupiler Pro League",BUL:"efbet Liga",CRO:"SuperSport HNL",CZE:"Fortuna Liga",DEN:"Superliga",EPL:"Premier League",L1:"Ligue 1",BUN:"Bundesliga",GRE:"Super League 1",ISR:"Ligat ha'Al",SA:"Serie A",NED:"Eredivisie",NOR:"Eliteserien",POL:"PKO BP Ekstraklasa",POR:"Liga Portugal",ROU:"SuperLiga",RUS:"Premier Liga",SCO:"Scottish Premiership",SRB:"Super liga Srbije",LLA:"LaLiga",SWE:"Allsvenskan",SUI:"Super League",TUR:"Süper Lig",UKR:"Ukrainian Premier League"};
const leagueLabel = c => LEAGUE_LABELS[c] || c;
const TOP5 = ['EPL','SA','BUN','L1','LLA'];
const PL_TOP6_TEAM_IDS = ['18','9','14','8','19','6'];
...
function updatePerfBanner(){
  const total = (MODE==='player') ? PLAYERS.length : TEAMS.length;
  const n = getCurrentData().length;
  perfTip.style.setProperty('--pct', total ? (n/total) : 0);
  perfTip.innerHTML = `<span class="meter-text">${n} ${MODE==='player' ? 'players' : 'teams'} selected</span>`;
  spinBtn.disabled = spinFab.disabled = (n===0);
}
...
function drawWheel(){
  ...
  for(let i=0;i<N;i++){
    ...
    ctx.fillStyle=data[i].primary_color||'#4f8cff'; ctx.fill();
  }
...
function spin(){
  ...
  spinning=true; spinBtn.disabled=spinFab.disabled=true;
  ...
  function step(now){
    ...
    if (p<1) requestAnimationFrame(step);
      ...
      spinning=false; spinBtn.disabled=spinFab.disabled=false;
      selectedIdx=idx; drawWheel(); showResult(idx);
    }
  }
...
function showResult(idx){
  const data=getCurrentData(); const item=data[idx]; if(!item) return;
  history.unshift(item); if(history.length>50) history=history.slice(0,50);
  localStorage.setItem('clubHistory', JSON.stringify(history));
  renderHistory(); openModal(item);
}
...
function blurEl(el, hug=false){
  if(!el) return; const w=wrap(el);
  if (hug) w.classList.add('hug'); else w.classList.remove('hug');
...
function unblurEl(el){
  if(!el) return; el.style.filter='';
  const w=el.parentElement; const o=w && w.querySelector('.reveal-overlay'); if(o) o.remove();
}
function addReveal(key, el, enabled, label, hug=false){
  const id=`reveal-${key}`; document.getElementById(id)?.remove();
  const w=wrap(el);
  if (enabled || modalReveal[key]){ unblurEl(el); w.classList.remove('hug'); return; }
  blurEl(el, hug);
  const b=document.createElement('button');
  b.id=id; b.type='button'; b.className='reveal-btn over'; b.textContent=`SHOW ${label.toUpperCase()}`;
  b.onclick=()=>{ modalReveal[key]=true; unblurEl(el); w.classList.remove('hug'); b.remove(); };
  w.appendChild(b);
}
...
function setMode(next){
  ...
  applyModeShowControls(); renderChips(); selectedIdx=-1; updatePerfBanner(); drawWheel();
}
...
function resolveClubName(id){
  ...
  const fallback={'9':'Manchester City','14':'Manchester United','18':'Chelsea','8':'Liverpool','19':'Arsenal','6':'Tottenham Hotspur','11':'Fulham','13':'Everton','15':'Aston Villa','20':'Newcastle United','27':'Burnley','29':'Wolverhampton Wanderers','51':'Crystal Palace','52':'AFC Bournemouth','63':'Nottingham Forest','71':'Leeds United','78':'Brighton & Hove Albion','236':'Brentford'};
  return fallback[key] || `Club #${key}`;
}
...
async function loadTeams(){
  ...
  TEAMS.forEach(t=>{ const id=String(t.team_id); CLUB_BY_ID.set(id,t.team_name); TEAM_BY_ID.set(id,t); });
}
...
function wire(){
  ...
  chipsWrap.addEventListener('change',()=>{ if(spinning) return; selectedIdx=-1; updatePerfBanner(); drawWheel(); });

  toggleMore.addEventListener('click',()=>{
    const hidden=chipsMore.hidden; chipsMore.hidden=!hidden;
    toggleMore.textContent = hidden ? (MODE==='player'?'Show fewer clubs':'Show fewer leagues')
                                    : (MODE==='player'?'Show more Premier League clubs':'Show more leagues');
    toggleMore.setAttribute('aria-expanded', String(!hidden));
  });

  // Quick picks
  qpAll.onclick=()=>{ const all=visibleCodesAll(); chipsWrap.querySelectorAll('input[type="checkbox"]').forEach(i=>i.checked=all.includes(i.value)); selectedIdx=-1; updatePerfBanner(); drawWheel(); };
  qpNone.onclick=()=>{ chipsWrap.querySelectorAll('input[type="checkbox"]').forEach(i=>i.checked=false); selectedIdx=-1; updatePerfBanner(); drawWheel(); };
  qpTop.onclick = ()=>{
    if(MODE==='player'){
...
(async function init(){
  try{
    await loadTeams();
    await loadPlayers();  // depends on teams for colors/names
