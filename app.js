/**
 * app.js — Football Spinner
 * - Team/Player modes (persisted)
 * - Robust “show on wheel” toggles
 * - Text/Logo layout with overlap avoidance + ellipsis
 * - Strict UI lock while spinning
 * - SPIN button centered on the canvas (positionSpinFab)
 */
...
let MODE = localStorage.getItem('fsMode') || 'team'; // 'team' | 'player'
...
function setMode(newMode) {
  ...
  if (MODE === 'team') {
    ...
    if (!PLAYERS) {
      loadPlayers().then(() => { selectedIdx = -1; drawWheel(); syncSpinDisabled(); })
                   .catch(err => { console.error('players.json load failed', err); drawWheel(); });
    } else {
      selectedIdx = -1;
      drawWheel();
    }
...
async function loadPlayers() {
  ...
  const mapTeamToLeague = {};
  TEAMS.forEach(t => {
    if (t?.team_name && t?.league_code) mapTeamToLeague[normalizeString(t.team_name)] = t.league_code;
  });
  ...
  PLAYERS = Array.isArray(data) ? data.map(p => {
    ...
    if (!league_code) {
      ...
      for (const c of clubCandidates) {
        ...
        if (mapTeamToLeague[norm]) { league_code = mapTeamToLeague[norm]; break; }
      }
    }
...
function renderPlayerListPreview(limit = 40) {
  ...
  PLAYERS.slice(0, limit).forEach(p => {
...
function drawWheel(){
  ...
  if (N === 0) {
    ...
    positionSpinFab(); // keep FAB centered even with no data
    return;
  }
  ...
  for (let i = 0; i < N; i++) {
    ...
    if (MODE === 'team') ctx.fillStyle = t.primary_color || '#4f8cff';
    else ctx.fillStyle = i % 2 === 0 ? 'rgba(16,24,40,0.92)' : 'rgba(10,16,28,0.92)';
    ctx.fill();
  }
  ...
  for (let i = 0; i < N; i++) {
    ...
    if (MODE === 'team') {
      const canShowName    = show.name && t.team_name && maxTextWidth >= PERF.minTextWidth && !hideText;
      const canShowStadium = show.stadium && t.stadium && maxTextWidth >= PERF.minTextWidth && !hideText;
      const canShowLogo    = show.logo && t.logo_url && logoHalf*2 >= PERF.minLogoBox && !hideLogos;
      ...
      const canShowLogo = show.logo && playerImgUrl && logoHalf*2 >= PERF.minLogoBox && !hideLogos;
      const canShowName = show.name && playerName && maxTextWidth >= PERF.minTextWidth && !hideText;
  ...
  ctx.restore();
  positionSpinFab(); // keep FAB locked to canvas center
}
...
function setResult(idx) {
  ...
  if (t) {
    if (MODE === 'team') {
      const leagueLabel = t.league_code && (LEAGUE_LABELS[t.league_code] || t.league_code) || '';
...
function renderHistory() {
  ...
  if (historyPlayersEl) historyPlayersEl.innerHTML = historyEl ? historyEl.innerHTML : '';
}
...
// Center the SPIN FAB on the canvas regardless of wrapper padding
function positionSpinFab() {
  if (!spinFab || !wheel) return;
  const wrap = wheel.parentElement || document.body;
  const wrapRect  = wrap.getBoundingClientRect();
  const wheelRect = wheel.getBoundingClientRect();
  const cx = wheelRect.left + wheelRect.width  / 2 - wrapRect.left;
  const cy = wheelRect.top  + wheelRect.height / 2 - wrapRect.top;
...
function sizeCanvas() {
  ...
  positionSpinFab(); // <— keep centered after resize/sizing
}

function syncSpinDisabled() {
  const hasAny = getCurrentData().length > 0;
  if (spinBtn) spinBtn.disabled = !hasAny;
  if (spinFab) spinFab.disabled = !hasAny;
  if (!hasAny && currentText) currentText.textContent = `Please select at least one ${MODE === 'player' ? 'player' : 'league'}.`;
}
...
function setupEventListeners() {
  if (chipsWrap) {
    chipsWrap.addEventListener('change', () => {
      ...
      drawWheel();
      syncSpinDisabled();
      if (perfTip) perfTip.textContent = `${getCurrentData().length} ${MODE === 'player' ? 'players' : 'teams'} selected`;
    });
  }
  if (toggleMore) {
  ...
  // Delegated listener for show-on-wheel controls
  document.addEventListener('change', (e) => {
    const target = e.target;
    if (!target) return;
    if (target.closest && (target.closest('#showOnWheel') || target.closest('.showopts'))) {
  ...
  if (resetHistoryBtn) resetHistoryBtn.addEventListener('click', () => { if (!spinning) { history = []; localStorage.removeItem('clubHistory'); renderHistory(); } });
  if (mClose) mClose.onclick = () => { if (!spinning) closeModal(); };
  if (backdrop) backdrop.addEventListener('click', e => { if(!spinning && e.target===backdrop) closeModal(); });
  ...
  window.addEventListener('resize', () => {
    ...
    resizeTO = setTimeout(() => { sizeCanvas(); drawWheel(); positionSpinFab(); }, 120);
  }, { passive: true });
}
...
fetch(`./teams.json?v=${Date.now()}`)
  ...
  .then(data => {
    ...
    sizeCanvas();
    setCheckedCodes(['EPL']);
    // Honor persisted mode
    if (MODE === 'player') setMode('player'); else setMode('team');
    setupEventListeners();
    drawWheel();
    positionSpinFab(); // initial placement
  })
  .catch(err => {
